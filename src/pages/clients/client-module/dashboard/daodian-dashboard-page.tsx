...
export default function DaodianDashboardPage({ clientId, guide, contacts }: Props) {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      const filtered = actList.filter((item) => item.businessType === '到店营销')
  ...
  const handleCreate = () => {
    navigate(`/clients/${clientId}/dashboard/create?businessType=到店营销`)
  }
    ...
    <div className={styles.tabContent}>
      ...
      <div className={styles.actionBar}>
        <Space size={12}>
          ...
          <Button
            onClick={() =>
              navigate(`/clients/${clientId}/dashboard/data-sources?businessType=到店营销`)
            }
            disabled={!clientId}
          >
            源数据管理
