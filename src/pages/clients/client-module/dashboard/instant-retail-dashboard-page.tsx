...
export default function InstantRetailDashboardPage({ clientId, guide, contacts }: Props) {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      const filtered = actList.filter((item) => item.businessType === '即时零售')
  ...
  const handleCreate = () => {
    navigate(`/clients/${clientId}/dashboard/create?businessType=即时零售`)
  }
    ...
    <div className={styles.tabContent}>
      ...
      <div className={styles.actionBar}>
        <Space size={12}>
          ...
          <Button
            onClick={() =>
              navigate(`/clients/${clientId}/dashboard/data-sources?businessType=即时零售`)
            }
            disabled={!clientId}
          >
            源数据管理
          ...
          <Button
            onClick={() => navigate(`/clients/${clientId}/dashboard/instant-retail-config`)}
            disabled={!clientId}
          >
            即时零售配置
          </Button>
